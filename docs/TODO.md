# TODO — 项目三自进化引擎：持续优化清单

> 项目三是多 Agent 自进化引擎，使命是持续优化任意目标项目（不限于 Bug 修复）。
> 优化覆盖：代码质量、测试覆盖、性能、架构、安全、文档、配置等所有维度。
> 每一轮必须有实际产出，没有待办就主动扫描目标项目找优化机会。
...
- [x] 任务ID: self_cleanup_deprecated_files
  描述: 扫描并删除 swarm 项目中已废弃/无用的文件，更新所有 import 引用
  验收标准:
    - ✅ 扫描所有 .py 文件，找出未被任何 import 引用的文件
    - ✅ 删除确认无用的文件（14个废弃文件清理完毕）
    - ✅ 更新所有 import 引用（无需更新——无import引用）
    - ✅ 确保删除后所有功能正常运行（语法检查通过）
  依赖: 无
  实际: 2026-05-19 19:52 — 协调者直接删除14个废弃文件+2个根目录测试文件
...
## ✅ P1 Bug Scan & Fix — 2026-05-19

- [x] 任务ID: scan_p1_codebase
  描述: 使用 code_review 扫描项目一全部58个Python文件，发现311个候选问题
  结论: ~95%为误报（n_plus_one = dict.get()误判, secret_leak = 测试token误判）

- [x] 任务ID: fix_critical_bug_async_context
  描述: 修复 chat_service.py 的 _build_messages sync 函数在 async 上下文内调用 build_context_sync 崩溃的Bug
  根因: _sync_build_context 检测到运行中事件循环时主动 raise RuntimeError
  修复: 将 _build_messages 转为 async def，使用 async build_context
  测试: 14/14 测试通过，71个关联测试全部通过
  Bug报告: bug_reports/p1_bug_scan_20260519.json

---

## ⬜ 待开发：code_review async/sync 边界检测

> 工单：tickets/optimize_code_review_async_sync_boundary.md

- [x] 任务ID: impl_async_sync_boundary_checker
  描述: 在 code_review.py 中新增 AsyncSyncBoundaryChecker，检测 sync-wrapper-raises / asyncio-run-in-loop / sync-calls-async 三类模式
  优先级: HIGH
  依赖: 无（项目三自身代码）
  验收: 能检测到 BUG-001 中的 _sync_build_context 模式
