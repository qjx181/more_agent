# Swarm Self-Evolving — 多 Agent 持续优化引擎

> 这不是一个「修 Bug」的系统。这是一个**持续优化项目**的多 Agent 自进化引擎。
> 它能覆盖的全维度：代码质量、测试覆盖、性能、架构、安全、文档、配置、异步化、死代码清理。
> 每轮自动扫描目标项目，发现一切可改进的地方，执行优化，验证效果，积累经验——然后下次更快。

> **项目代号：** swarm-self-evolve
> **创建日期：** 2026-05-15
> **当前轮次：** Round 46（cronjob 暂停中，2026-05-19）
...
# 2. 配置项目一目录（用于 git 同步 + Bug 管道扫描）
export PROJECT1_DIR=/mnt/c/Users/qjx/Desktop/agent-自进化版/项目一cursor版本/在线部分
# 或在 config.yaml 中设置: swarm.project1_dir: "/path/to/project1"
# 或在 .env 中设置: PROJECT1_DIR=/path/to/project1

# 3. 启动 Hermes 守护进程（tmux 后台运行）
bash start_hermes_daemon.sh
...
# 5. 查看产出
cat logs/self_evolve.log  # 查看进化日志
cat state.json            # 查看当前状态

# 6. 设置系统 cron（每30分钟自动运行）
crontab -e
# 添加：*/30 * * * * python3 /mnt/f/项目三：多Agent/cron_trigger.py >> /mnt/f/项目三：多Agent/logs/cron_stdout.log 2>&1
```
